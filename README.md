...
- 🖼️ **Canvas 2D** - 波形描画
- 🔒 **Cross-Origin Isolated** - FFmpeg.wasm動作
...
### ⚡ パフォーマンス仕様
- **Canvas描画**: 60fps目標（録画時30fps）
- **音声解析**: 2048サンプル/フレーム
- **メモリ使用量**: 音声ファイルサイズの約3-5倍
- **FFmpeg処理**: WebAssembly最適化済み
...
#### コア技術スタック
- **React 18** + **TypeScript** - モダンReact開発
- **Vite** - 高速ビルドツール
- **shadcn/ui** + **Radix UI** - アクセシブルUIコンポーネント
- **Tailwind CSS** - ユーティリティファーストCSS
- **ESLint** - コード品質管理 (Flat Config)

#### 音声・動画処理
- **Web Audio API** - リアルタイム音声解析
- **Canvas 2D API** - 高性能波形描画
- **MediaRecorder API** - ブラウザ内録画
- **FFmpeg.wasm** - WebM→MP4変換

#### 設定・分析
- **LocalStorage** - パラメーター永続化
- **Google Analytics 4** - 使用状況分析
- **TypeScript Path Aliases** - `@/*` import

### 主要な依存関係
```json
{
  "@ffmpeg/ffmpeg": "^0.12.7",      // 動画変換
  "@radix-ui/react-*": "^1.0.0",   // UI基盤
  "class-variance-authority": "^0.7.1", // CSS管理
  "tailwind-merge": "^2.5.4",      // クラス最適化
